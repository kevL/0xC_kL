...
void Bar::draw()
{
	...
	if (_invert == true)
		...
		drawRect(&rect, static_cast<Uint8>(_color + 6u)); // was +4 but red is wonky.
	...
	if (_invert == true)
	{
		if (_secondOnTop == true)
		{
			...
			drawRect(&rect, static_cast<Uint8>(_color + 4u));

			rect.w = static_cast<Uint16>(Round(width2));
			rect.y = static_cast<Sint16>(rect.y + _offSecond_y);
			drawRect(&rect, static_cast<Uint8>(_color2 + 4u));
		}
		else
		{
			rect.w = static_cast<Uint16>(Round(width2));
