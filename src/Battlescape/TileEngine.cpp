...
//									if (blockType > 0)		// kL: -1 - do NOT crop trajectory (ie. hit content-object)
											...
											++i)
									{
												...
												&& tile->getMapData(O_OBJECT)->getBigwall() != BIGWALL_E_S))
										{
											...
											if (tileEdge != nullptr)
											{
												if (tileEdge->getMapData(O_OBJECT) != nullptr
													...
													&& tile->getMapData(O_OBJECT)->getBigwall() != BIGWALL_WEST)))
										{
											tileEdge = _battleSave->getTile(Position( // show Tile WEST
																				...
																				posTrj.z));
											if (tileEdge != nullptr
												&& tileEdge->getMapData(O_OBJECT) != nullptr
												&& (tileEdge->getMapData(O_OBJECT)->getBigwall() == BIGWALL_BLOCK
...
{
	//Log(LOG_INFO) << "TileEngine::canTargetUnit()";
	const bool hypothetical = (targetUnit != nullptr);
...
void TileEngine::detonate(Tile* const tile) const
{
			...
			--i)
	{
		...
		// kL_begin:
		if (tile->getMapData(O_OBJECT) != nullptr									// if tile has object
		...
		while (tiles[i]->getMapData(partType) != nullptr
 ...
 * @param voxelType		- pointer to a type of voxel at which this parabola terminates (default nullptr)
