 ...
 *
 */
HQX_API void HQX_CALLCONV hq4x_32_rb(
		const uint32_t* sp,
		uint32_t srb,
		uint32_t* dp,
		uint32_t drb,
		...
		int Yres)
{
	int
		...
		prevline,
		nextline,
		dpL (static_cast<int>(drb >> 2)),
		spL (static_cast<int>(srb >> 2)),
		pattern,
		flag;
	const uint8_t
		* sRowP (reinterpret_cast<const uint8_t*>(sp)),
		* dRowP (reinterpret_cast<const uint8_t*>(dp));
	uint32_t
		yuv1,yuv2,
		w[10u];
	...
	for (j = 0; j < Yres; j++)
	{
		...
		sRowP += srb;
		sp = reinterpret_cast<const uint32_t*>(sRowP);
 ...
 *
 */
HQX_API void HQX_CALLCONV hq4x_32(
		const uint32_t* sp,
		uint32_t* dp,
		int Xres,
		int Yres)
{
	uint32_t rowBytesL (static_cast<uint32_t>(Xres * 4));
	hq4x_32_rb(
			sp,
			rowBytesL,
			dp,
