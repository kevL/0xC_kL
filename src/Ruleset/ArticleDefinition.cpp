...
//	_type_id	= (UfopaediaTypeId)node["type_id"].as<int>(_type_id); // This got set in the cTor.
	...
	if (_listOrder == 0)		// if there's no defined listorder in YAML
 ...
 * Constructor.
 */
ArticleDefinitionRect::ArticleDefinitionRect()
	:
		x(0),
		y(0),
		width(0),
 ...
 * Constructor (only setting type of base class).
 */
ArticleDefinitionTextImage::ArticleDefinitionTextImage()
	:
